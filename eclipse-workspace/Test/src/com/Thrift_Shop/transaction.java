...
public class transaction extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (flag == 1) {
			...
			String createView = "CREATE VIEW `" + viewName
					+ "` AS select Item.i_id,Item.s_id,Item.stock,Contains.quantity,Contains.partial_amount,Item_Detail.title from Item INNER JOIN (Contains INNER JOIN Item_Detail using(i_id))using(i_id) where o_id=? ";
		...
		{
			Query_ViewCart q = (Query_ViewCart)session.getAttribute("result");
			int qty = q.getQuantity(0);
			int iid = (int)session.getAttribute("viewable_iid");
			System.out.println("iID" + iid);
			//int qty = (int)session.getAttribute("incaseofbuynowQuantity");
			String selectres = "Select @result";
			String callproc = "call directBuyNow(?,?,?,@result)";
			String showres = "Select @result AS RESULT";
			int res=0;
			try
			{
				Connection con=DatabaseConnection.initializeDatabase();
				PreparedStatement st=con.prepareStatement(selectres);
				st.executeQuery();
				st=con.prepareStatement(callproc);
				st.setInt(1, b_id);
				st.setInt(2, iid);
				st.setInt(3, qty);
				st.executeUpdate();
				st=con.prepareStatement(showres);
				ResultSet rs = st.executeQuery();
				rs.next();
				res=rs.getInt("RESULT");
				if(res == 0)
				{
					request.setAttribute("itemqtyError","Stock unavailable");
					request.getRequestDispatcher("BuyNow.jsp").forward(request, response);
				}
				else if(res == 1)
				{
